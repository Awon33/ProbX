...
const DiceRoll = () => {
  ...
  const getNextQuestion = (targetDifficulty, excludeIds, currentScore) => {
    ...
    let available = diceQuestions.filter(q => 
    ...
    if (available.length === 0) {
      ...
      for (const altDiff of alternativeDifficulties[targetDifficulty]) {
        available = diceQuestions.filter(q => 
    ...
    if (available.length === 0) {
      ...
      if (excludeIds.length >= diceQuestions.length * 0.8) { // 80% of questions used
        available = diceQuestions.filter(q => q.difficulty === targetDifficulty);
        if (available.length === 0) {
          available = diceQuestions; // All questions as fallback
        }
    ...
    const fallbackQuestion = diceQuestions[Math.floor(Math.random() * diceQuestions.length)];
